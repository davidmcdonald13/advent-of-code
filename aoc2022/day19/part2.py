...
class Graph:
    ...
    def solve(self, state, mins_remaining):
        ...
        pq.put((-theoretical_max, 0, mins_remaining, state))
        while not pq.empty():
            _theoretical_max, _accrued, mins_remaining, state = pq.get()
            ...
            # do nothing, but only if there's something we're saving up for
            if len(future_states) < 4:
                future_states.append(state.copy(resources = state.resources + new_resources))

            for future_state in future_states:
                if (future_state, mins_remaining - 1) in visited:
                    continue
...
def solve(line):
    ...
    result = graph.solve(State.default(), 32)
