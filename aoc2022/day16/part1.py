...
class Memo:
    ...
    def dfs(self, curr, open_valves, mins_remaining):
        key = (curr, tuple(open_valves), mins_remaining)
    ...
    def _dfs(self, curr, open_valves, mins_remaining):
        ...
        if not curr in open_valves and self.valves[curr] > 0:
            new_open_valves = {x for x in open_valves}
            new_open_valves.add(curr)
            result = current_rate + self.dfs(curr, new_open_valves, mins_remaining - 1)

        for dest in self.tunnels[curr]:
            result = max(result, current_rate + self.dfs(dest, open_valves, mins_remaining - 1))
