...
fn main() {
    ...
    let divisor_product: i64 = monkeys.iter().map(|item| item.divisor_check).reduce(|a, b| a * b).unwrap();
    ...
    for _ in 0..10000 {
        ...
        for i in 0..count {
            ...
            let queue: Vec<(usize, i64)> = monkey.items.iter().map(|item| {
                let new_val: i64 = (monkey.operation)(*item, monkey.op_val) % divisor_product;
