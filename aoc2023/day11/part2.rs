...
fn main() {
    ...
    let result = run(contents, 1000000);
    println!("result: {}", result);
}

fn run(input: String, expansion_factor: usize) -> usize {
    ...
    for i in 0..galaxies.len() {
        for j in (i+1)..galaxies.len() {
            ...
            let row_dist: usize = get_size(&row_iter) + empty_rows.iter().filter(|x| row_iter.contains(x)).count() * (expansion_factor - 1);
            let col_dist: usize = get_size(&col_iter) + empty_cols.iter().filter(|x| col_iter.contains(x)).count() * (expansion_factor - 1);
